...
# Test that the extraction pipeline still works
...
for pdf_path, format_name in test_cases:
    ...
    try:
        ...
        # Check success criteria
        has_rows = rows_extracted > 0
        has_subtotal = financials.get("subtotal", 0) > 0
        has_deduction = "total_deduction" in financials
        has_net = financials.get("net_payable", 0) > 0
        
        print(f"  Rows extracted:     {rows_extracted} {'✅' if has_rows else '❌'}")
        print(f"  Subtotal:           {financials.get('subtotal', 0):.2f} {'✅' if has_subtotal else '❌'}")
        print(f"  Deduction:          {financials.get('total_deduction', 0):.2f} {'✅' if has_deduction else '❌'}")
        print(f"  Net Payable:        {financials.get('net_payable', 0):.2f} {'✅' if has_net else '❌'}")
        print(f"  Deduction Source:   {financials.get('deduction_source', 'unknown')}")
