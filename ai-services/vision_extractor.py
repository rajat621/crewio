...
# Debug-flow fix: this file previously called int(os.getenv("X", "default"))
# directly - that pattern only falls back to "default" when the variable is
# entirely absent, not when it's present-but-empty (a real, observed
# production case: VISION_RETRY_429="" caused int("") -> ValueError,
# crashing the whole extraction request instead of just skipping the
# retry). _to_int/_to_float already exist in config_runtime.py for exactly
# this reason (try/except around the parse, empty string falls back to
# default) - reused here rather than duplicating the same helper.
from config_runtime import _to_int, _to_float
...
def _call_gemini(images: List[Any]) -> Optional[Dict[str, Any]]:
    ...
    timeout_s = _to_int(os.getenv("VISION_TIMEOUT_S"), 120, minimum=1)
    ...
    max_retries = _to_int(os.getenv("VISION_RETRY_429"), 0, minimum=0)
    backoff_s = _to_float(os.getenv("VISION_RETRY_BACKOFF_S"), 2.0, minimum=0.0)
...
def extract_vision(pdf_path: str) -> NormalizedInvoice:
    ...
    max_pages = _to_int(os.getenv("VISION_MAX_PAGES"), 10, minimum=1)
