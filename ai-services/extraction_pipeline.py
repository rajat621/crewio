...
def run_extraction_pipeline(
    ...
    def _finalize(result: NormalizedInvoice) -> NormalizedInvoice:
        result.warnings.append(f"layout:{layout_result.layout_type.value}")
        result.warnings.append(f"strategy:{strategy.reason}")
        return _apply_post_processing(result, review_mode=review_mode)

    # -----------------------------------------------------------------------
    # STEP 2 — Route extraction per strategy, STEP 4 — Fallback logic
    # -----------------------------------------------------------------------
    stage_order = [strategy.primary] + [s for s in strategy.fallbacks if s != strategy.primary]

    for stage in stage_order:
        if stage == ExtractionStage.NATIVE:
            logger.info("step2_route native_pdf_extraction reason=%s", strategy.reason)
            try:
                native_result = extract_native(pdf_path)
                if native_result.is_valid:
                    logger.info(
                    ...
                    logger.info(
                        "step2_native insufficient rows=%d error=%s — trying next stage",
                        len(native_result.invoice_rows), native_result.error,
                    )
            except Exception as exc:
                logger.warning("step2_native failed: %s — trying next stage", exc)
                native_result = None
        ...
        elif stage == ExtractionStage.VISION:
            ...
            try:
                ...
                logger.warning(
                    "VISION FAILED -> FALLING BACK FOR FILE: %s",
                    pdf_path,
                )
                logger.warning("step2_vision failed: %s — trying next stage", exc)
                vision_result = None

            if vision_result is not None and _vision_result_is_usable(vision_result):
                final = vision_result
                break
