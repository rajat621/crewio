...
from .row_clusterer import cluster_rows
...
class GridReconstructor:
    ...
    def reconstruct(
        ...
        if len(row_boundaries) < 3:
            row_boundaries = self._boundaries_from_row_clusters(ocr_cells, limit=h)
        if len(col_boundaries) < 3:
            col_boundaries = self._boundaries_from_column_clusters(ocr_cells, limit=w)
        ...
        for raw_cell in ocr_cells:
            ...
            grid_cells.append(
                GridCell(
                    ...
                    text=text,
                    confidence=conf,
                    page_number=self._optional_int(cell.get("page_number")),
                )
            )
    ...
    def _boundaries_from_row_clusters(self, cells: Sequence[OCRCell], limit: int) -> List[int]:
        """Fallback segmentation based on clustered OCR rows."""

        clusters = cluster_rows(cells)
        points: List[int] = [0, limit]
        for row in clusters:
            points.extend([int(row.y_top), int(row.y_bottom)])
        merged = self._merge_nearby_points(sorted(points), self.config.boundary_merge_tolerance)
        return sorted(set([0, limit] + merged))

    def _boundaries_from_column_clusters(self, cells: Sequence[OCRCell], limit: int) -> List[int]:
        """Fallback segmentation based on clustered OCR columns."""

        clusters = cluster_columns(cells)
        points: List[int] = [0, limit]
        for col in clusters:
            points.extend([int(col.x_left), int(col.x_right)])
        merged = self._merge_nearby_points(sorted(points), self.config.boundary_merge_tolerance)
        return sorted(set([0, limit] + merged))
    ...
    def _normalize_cell(cell: OCRCell) -> OCRCell:
        ...
        output = dict(cell)
        output["x"] = int(cell.get("x", 0))
        output["y"] = int(cell.get("y", 0))
        output["w"] = int(cell.get("w", 0))
