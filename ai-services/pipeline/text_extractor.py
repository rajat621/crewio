...
def _ocr_full_text(pdf_path: str) -> str:
    ...
    try:
        ...
        for page_idx, preprocessed in enumerate(preprocessed_pages, 1):
            ...
            tokens = ocr_provider.image_tokens(preprocessed.image, page_number=page_idx)

            if not tokens:
                continue

            tokens.sort(key=lambda item: (int(item.get("y", 0)), int(item.get("x", 0))))
...
def _downscale_for_ocr(image: np.ndarray) -> Tuple[np.ndarray, bool]:
    max_side = _env_int("MAX_OCR_IMAGE_SIDE", 2600, minimum=1)
    max_pixels = _env_int("MAX_OCR_IMAGE_PIXELS", 9000000, minimum=1)
    h, w = image.shape[:2]
    if h <= 0 or w <= 0:
        return image, False
    current_pixels = h * w
                ...
                try:
                    ...
                    try:
                        ...
                        if prov and prov.available():
                            tokens = prov.image_tokens(crop, page_number=page_idx) or []
                            tokens_sorted = sorted(tokens, key=lambda t: (int(t.get("y", 0)), int(t.get("x", 0))))
...
def extract_text_pdf(
    ...
    try:
        ...
        if route_to_ocr:
            ...
            if ocr_table_text:
                max_ocr_text_chars = _env_int("MAX_OCR_TEXT_CHARS", 45000, minimum=1000)
