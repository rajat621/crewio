...
_AMOUNT_ALIASES = {
    "amount", "subtotal", "earned", "gross", "payable", "value",
    "amount aed", "total amount",
}
# "total" alone used to be in the set above, matched as a substring against
# every header cell, and silently claimed columns it had no business
# claiming - "TOTAL DEDUCTION", "TOTAL ABSENT", and critically, a bare
# "TOTAL" column that actually holds an employee's total HOURS in an
# attendance-matrix layout (rate + day-by-day columns, no explicit "hours"
# header at all). That single mapping bug is what produced the doubled
# rows in invoice_draft.py's build_draft: the same project+trade got one
# row from this table (hours misread as amount, amount back-derived as
# amount/rate) and one correct row from a separate summary table later in
# the same document, and group_rows_for_invoice summed both under the same
# key. See _resolve_bare_total_column below for the narrow, exact-match
# replacement.
...
def _detect_column_mapping(header_row: List[str]) -> Dict[str, int]:
    ...
    for idx, cell in enumerate(header_row):
        ...
        if _match_semantic(h, _TRADE_ALIASES) and "trade" not in mapping:
            ...
            _match_semantic(h, _AMOUNT_ALIASES)
            and "amount" not in mapping
            and not _match_semantic(h, _DEDUCTION_KEYWORDS)
        ):
            mapping["amount"] = idx
        elif _match_semantic(h, _PROJECT_ALIASES) and "project" not in mapping:
            mapping["project"] = idx

    # A header cell that is EXACTLY "total" (nothing else - "total deduction"
    # and "total absent" do not qualify) and hasn't already been claimed by
    # anything above is ambiguous on its own, but not in context: an
    # attendance-matrix table (rate column present, day-by-day columns
    # after it, no dedicated hours/amount header at all) always uses a bare
    # TOTAL to mean total hours for that employee - the amount is implicit
    # (hours x rate), never printed as its own column. Only resolve it that
    # way; leave it unmapped otherwise rather than guessing.
    if "hours" not in mapping and "rate" in mapping:
        for idx, cell in enumerate(header_row):
            if _normalize(str(cell or "")) == "total":
                mapping["hours"] = idx
                break
...
def extract_native(pdf_path: str) -> NormalizedInvoice:
    ...
    all_rows: List[NormalizedInvoiceRow] = []
    full_text_parts: List[str] = []
    # Running (project, trade) -> amount total across tables kept so far.
    # Some documents print both a detailed day-by-day breakdown AND a
    # compact recap table of the exact same project/trade totals later on
    # (typically as a final summary page). If a newly-parsed table's rows
    # reproduce totals already accumulated here to the cent, it's that
    # recap, not new data - keeping both would double-count every group.
    accumulated_amounts: Dict[Tuple[str, str], float] = {}
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page_idx, page in enumerate(pdf.pages, 1):
                ...
                for table in tables:
                    ...
                    table_totals: Dict[Tuple[str, str], float] = {}
                    for r in rows:
                        trade_norm = _normalize(r.description)
                        if trade_norm in {"total", "totals", "grand total", "sub total", "subtotal"}:
                            continue
                        # .replace(" ", "") absorbs occasional pdfplumber
                        # kerning artifacts that space out individual letters
                        # of a trade name ("M A SON" for "MASON") in one
                        # table but not the other - without it, those rows
                        # never matched their recap-table counterpart and
                        # the whole recap table was kept as if it were new.
                        key = (r.project.strip().upper(), trade_norm.replace(" ", ""))
                        table_totals[key] = round(table_totals.get(key, 0.0) + r.amount, 2)

                    is_recap = bool(table_totals) and all(
                        key in accumulated_amounts
                        and abs(accumulated_amounts[key] - total) < 0.05
                        for key, total in table_totals.items()
                    )

                    if is_recap:
                        logger.info(
                            "native_extract skipping recap table page=%d rows=%d "
                            "(totals match data already extracted from an earlier table)",
                            page_idx, len(rows),
                        )
                        continue
                    ...
                    all_rows.extend(rows)
                    for key, total in table_totals.items():
                        accumulated_amounts[key] = accumulated_amounts.get(key, 0.0) + total
