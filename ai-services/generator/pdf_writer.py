...
def _draw_fallback_branding(c: canvas.Canvas, profile: CompanyProfile) -> None:
    """Draw a minimal branded background when no template asset is available."""
    page_w, page_h = A4
    left_bar_w = 28 * mm
    top_bar_h = 28 * mm
    footer_h = 16 * mm
    ...
    # Left branding strip
    c.setFillColor(colors.HexColor("#1e3f70"))
    c.rect(0, 0, left_bar_w, page_h, stroke=0, fill=1)
    c.setFillColor(colors.HexColor("#2f588b"))
    c.rect(0, 0, 4 * mm, page_h, stroke=0, fill=1)
    c.saveState()
    c.translate(8 * mm, page_h / 2)
    c.rotate(90)
    c.setFillColor(colors.HexColor("#5f7ea7"))
    c.setFont("Helvetica-Bold", 22)
    c.drawCentredString(0, 0, (profile.name or "ALQASER ALSATEA TECH CONT").upper())
    c.restoreState()

    # Top header band
    c.setFillColor(colors.HexColor("#edf4fb"))
    c.rect(left_bar_w, page_h - top_bar_h, page_w - left_bar_w, top_bar_h, stroke=0, fill=1)
    c.setStrokeColor(colors.HexColor("#c9d8ea"))
    c.line(left_bar_w, page_h - top_bar_h, page_w, page_h - top_bar_h)
    c.setFillColor(colors.HexColor("#1a1a1a"))
    c.setFont("Helvetica-Bold", 14)
    c.drawCentredString((left_bar_w + page_w) / 2, page_h - 10 * mm, (profile.name or "ALQASER ALSATEA TECH CONT").upper())
    c.setFont("Helvetica", 9)
    c.drawCentredString((left_bar_w + page_w) / 2, page_h - 15 * mm, "Together we build")

    # Footer band with owner details
    c.setStrokeColor(colors.HexColor("#d6dce5"))
    c.line(left_bar_w + 2 * mm, footer_h, page_w - 2 * mm, footer_h)
    c.setFillColor(colors.HexColor("#1a1a1a"))
    c.setFont("Helvetica-Bold", 6.8)
    c.drawString(left_bar_w + 2 * mm, 7.5 * mm, (profile.name or "ALQASER ALSATEA TECH CONT").upper())
    contact_parts = [s for s in [profile.mobile, profile.email, profile.website] if s]
    if contact_parts:
        c.setFont("Helvetica", 6.4)
        c.drawRightString(page_w - 2 * mm, 7.5 * mm, " | ".join(contact_parts))

    # Reserve a small white margin above the footer so it reads like the screenshots.
    c.setFillColor(colors.white)
    c.rect(left_bar_w, footer_h, page_w - left_bar_w, 1.8 * mm, stroke=0, fill=1)
...
def generate_invoice_pdf(
    ...
    file_id      = run_id or str(uuid.uuid4())
    output_path  = os.path.join(output_dir, f"tax-invoice-{file_id}.pdf")
    ...
    if not template_asset:
        _draw_fallback_branding(c, profile)
