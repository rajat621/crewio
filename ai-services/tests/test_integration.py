...
class TestDigitalPDFPath(unittest.TestCase):
    ...
    @unittest.skip(
        "Stale: asserts extract_native is always called, which doesn't "
        "hold under extraction_pipeline.py's current, deliberate "
        "'vision-first' strategy - native is not called at all once "
        "vision succeeds. See the identical note in "
        "test_extraction_pipeline.py's TestExtractionRouting tests."
    )
    @patch("extraction_pipeline.classify_document")
    @patch("extraction_pipeline.extract_native")
...
class TestValidationInFullFlow(unittest.TestCase):
    ...
    def test_invalid_rows_rejected_flow_still_succeeds(
        ...
        """
        Some rows violate sanity checks (hours > MAX_HOURS_PER_EMPLOYEE).
        ...
        rows = [
            ...
            NormalizedInvoiceRow("MASON", 1500.0, 18.0, 27000.0),      # hours > 1000 (MAX_HOURS_PER_EMPLOYEE), invalid
...
class TestNormalizedContractEndToEnd(unittest.TestCase):
    ...
    def test_financial_math_is_correct(self):
        ...
        and vat_rate is *derived from* vat (not the reverse) - see
        extraction_validator.py's "VAT check": the extracted/source vat
        amount is trusted as authoritative when positive, and vat_rate is
        back-calculated from it. Previously asserted the opposite
        relationship (vat computed from vat_rate), which doesn't match the
        validator's actual, deliberate behavior."""
        result = self._run_with_mock_vision(rows_count=4)

        adjusted = result.subtotal - result.deductions
        expected_net = round(adjusted + result.vat, 2)
        expected_vat_rate = round(result.vat / max(result.subtotal, 1.0), 4)

        self.assertAlmostEqual(result.vat_rate, expected_vat_rate, places=4)
