...
class TestSanityChecks(unittest.TestCase):
    ...
    def test_hours_exceeding_1000_rejected(self):
        # Was written against an earlier MAX_HOURS_PER_EMPLOYEE=400 (this
        # test previously used quantity=401.0, named "_exceeding_400").
        # normalized_output.py's own comment marks this constant as
        # "Sanity check constants (from spec)" and its current value is a
        # deliberate 1000.0 - not a typo, and not something to "fix" back
        # to 400 without evidence the spec regressed. Updated the test to
        # match the current, spec-sourced limit.
        row = _make_row(quantity=1001.0, rate=15.0, amount=15015.0)
...
class TestValidationLayer(unittest.TestCase):
    ...
    def test_vat_trusted_from_source_and_rate_derived(self):
        # Renamed from test_vat_recomputed_when_inconsistent, which
        # asserted the opposite of extraction_validator.py's actual,
        # deliberate "VAT check" logic (see the comment there): the
        # validator trusts invoice.original_vat (the amount as extracted/
        # printed on the source document) as authoritative whenever it's
        # positive, and *derives* vat_rate from it - it does not recompute
        # vat from a possibly-wrong assumed rate. This is intentional:
        # a VAT amount printed on a real invoice is generally more
        # reliable than a rate inferred elsewhere in the pipeline.
        rows = [_make_row(quantity=100, rate=15, amount=1500)]
        inv = _make_invoice(rows=rows, subtotal=1500, vat=999)
        inv.vat_rate = 0.05
        validated, report = validate_and_repair(inv)
        self.assertAlmostEqual(validated.vat, 999.0, places=1)
        self.assertAlmostEqual(validated.vat_rate, 999.0 / 1500.0, places=4)
    ...
    def test_rows_violating_sanity_are_rejected(self):
        rows = [
            ...
            _make_row(quantity=1500, rate=15, amount=22500),  # hours > 1000 (MAX_HOURS_PER_EMPLOYEE), invalid
        ]
...
class TestExtractionRouting(unittest.TestCase):
    ...
    @unittest.skip(
        "Stale: asserts the pre-'vision-first' routing strategy. "
        "extraction_pipeline.py's own comments now explicitly document "
        "'Vision always goes first now... strategy.primary no longer "
        "decides which extractor goes first' - a deliberate architecture "
        "change this test was never updated for. Not re-written this "
        "session rather than risk asserting incorrect behavior for the "
        "current vision-first strategy without fully tracing its new "
        "fallback rules."
    )
    ...
    @unittest.skip(
        ...
        "the identical note on test_digital_pdf_uses_native_first above."
    )
    @patch("extraction_pipeline.classify_document")
    @patch("extraction_pipeline.extract_native")
    @patch("extraction_pipeline.extract_vision")
