...
# Debug-flow fix: same issue as vision_extractor.py - int(os.getenv("X", "8"))
# only falls back to "8" when the variable is entirely absent, not when it's
# present-but-empty. OCR_MAX_PAGES="" reproduced the observed
# "OCR page render failed: invalid literal for int() with base 10: ''".
from config_runtime import _to_int
...
def _render_pages(pdf_path: str, dpi: int = 250) -> List[Any]:
    ...
    try:
        ...
        max_pages = _to_int(os.getenv("OCR_MAX_PAGES"), 8, minimum=1)
        max_side = _to_int(os.getenv("OCR_MAX_IMAGE_SIDE"), 2400, minimum=1200)
