...
_HEADER_ALIASES = {
    "trade": ("trade", "designation", "worker trade", "labor category", "labour category"),
    "rate": ("rate", "unit price", "unitprice", "price", "net rate", "daily rate", "hourly rate"),
    "hours": ("hour", "hours", "no of hours", "total hours", "qty", "quantity"),
    "amount": ("amount", "total amount", "amount aed", "value", "total aed", "line total"),
    "project": ("project", "project id", "project no", "project code", "projectno"),
    "employee": ("employee", "name", "id", "id no", "i d no", "employee name", "worker name"),
}
...
def _parse_summary_rows(table: Sequence[Sequence[str]], table_index: int) -> List[InvoiceRow]:
    ...
    for row in table[start_idx:]:
        ...
        if amount <= 0 and nums:
            amount = nums[-1]
        if rate <= 0 and "rate" in mapping and len(nums) >= 2:
            rate = nums[-2]
        if hours <= 0 and len(nums) >= 3:
            hours = nums[-3]
        if hours <= 0 and rate > 0 and amount > 0:
        ...
        if len(nums) == 2 and (hours <= 0 or rate <= 0 or amount <= 0):
            n1, n2 = nums[0], nums[1]
            if n1 > 0 and n2 > 0:
                hours = hours if hours > 0 else n1
                amount = amount if amount > 0 else n2
                rate = rate if rate > 0 else round(amount / hours, 4)
                rate_derived = rate_derived or "rate" not in mapping

        if rate <= 0 and hours > 0 and amount > 0:
        ...
        if expected > 0 and not rate_derived:
...
def route_document_tables(
    ...
    for idx, table in enumerate(tables):
        ...
        elif ttype == TableType.PROJECT_SUMMARY_TABLE:
            parsed = parse_financial_summary_table(table)
            payload.financials = parsed.financials
            payload.rows = _parse_summary_rows(table, table_index=idx)
            payload.parser_name = "summary_table_parser"

        elif ttype in {TableType.ATTENDANCE_TABLE, TableType.OVERTIME_TABLE}:
