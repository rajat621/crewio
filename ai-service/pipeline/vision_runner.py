...
def extract_using_vision(pdf_path: str, run_id: Optional[str] = None) -> ExtractionResult:
    ...
    try:
        ...
        print("\nAFTER _rows_from_payload")
        print("rows =", len(rows))
        for r in rows:
            print(vars(r))

        rows2 = _aggregate_primary_rows(rows)

        print("\nAFTER _aggregate_primary_rows")
        print("rows =", len(rows2))
        for r in rows2:
            print(vars(r))

        rows3 = _hard_validate_rows(rows2, warnings)

        print("\nAFTER _hard_validate_rows")
        ...
        if isinstance(raw.get("deductions"), dict):
            deduction_value = _to_float(
                raw["deductions"].get("total")
            )
        else:
            deduction_value = _to_float(
                raw.get("deductions")
        ...
        fin = InvoiceFinancials(
            ...
            net_payable=max(
                0.0,
                _to_float(raw.get("net_total"))
            ),
            deduction_source="vision"
        )
...
def _rows_from_payload(payload: Dict[str, Any], warnings: List[str]) -> List[InvoiceRow]:
    ...
    elif isinstance(payload.get("employees"), list):
        source_rows = payload["employees"]
    ...
    for idx, item in enumerate(source_rows, 1):
        ...
        if not trade:
            warnings.append(
                f"vision_row_rejected:missing_trade:{idx}"
            )
            continue
        ...
        if hours <= 0 or rate <= 0 or amount <= 0:
            warnings.append(
                f"vision_row_rejected:missing_numeric:{idx}"
            )
            continue
