...
def generate_invoice_pdf(
    ...
    raw_rows: List[Dict[str, Any]] = extraction.get("rows", [])

    logger.info(
        "Row lifecycle | stage=raw_input | count=%d",
        len(raw_rows),
    )

    grouped_rows = group_rows_for_invoice(raw_rows)

    rows = []

    for idx, trade_group in enumerate(grouped_rows, start=1):
        trade = trade_group.to_dict() if hasattr(trade_group, "to_dict") else dict(trade_group)
        source_employee_ids = list(getattr(trade_group, "source_employee_ids", []) or trade.get("source_employee_ids", []) or [])

        rows.append({
            "trade": trade["trade"],
            "project_id": trade.get("project_id", "") or "",
            "employee_id": ";".join(source_employee_ids),
            "employee_name": "",
            "hours": trade.get("total_hours", trade.get("hours", 0.0)),
            "rate": trade.get("rate", trade.get("blended_rate", 0.0)),
            "amount": trade.get("total_amount", trade.get("amount", 0.0)),
            "source_employee_ids": source_employee_ids,
        })

    logger.info(
        "Row lifecycle | stage=grouped_invoice | count=%d | rows=%s",
        len(rows),
        rows,
    )
    ...
    # Invoice layout: prefer explicit override, otherwise infer from the row shape.
    fmt = str(extraction.get("format", "mcc") or "mcc").lower()
    if company_data.get("invoice_layout"):
        layout = str(company_data.get("invoice_layout") or "mcc").lower()
    else:
        has_project = any(str(row.get("project_id") or "").strip() for row in rows)
        has_employee = any(str(row.get("employee_id") or "").strip() for row in rows)
        if has_project:
            layout = "mcc"
        elif has_employee:
            layout = "bkc"
