...
class ApiClient {
  ...
  late Dio _dio;
  // Single-flight refresh: if several requests 401 around the same time
  // (e.g. Home screen firing off profile + salary + notifications calls
  // together), they all await this SAME Future instead of each racing to
  // refresh independently - the previous version let only the first 401
  // attempt a refresh and had every OTHER concurrent 401 immediately clear
  // tokens (logging the user out) instead of waiting for that refresh to
  // finish.
    ...
    _dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) async {
          // tokenService.getToken() is internally timeout-guarded (see
          // TokenService) against flutter_secure_storage/Android Keystore
          // hangs, so this can never block the request from being sent.
        ...
        onError: (error, handler) async {
          ...
          if (error.response?.statusCode == 401 && !isAuthRoute) {
            try {
              // Kick off a refresh only if one isn't already running; every
              // concurrent 401 (including this one, if it's not first)
              // awaits that same Future rather than starting its own.
              _refreshFuture ??= _tryRefreshToken().whenComplete(() {
                _refreshFuture = null;
              });
            ...
            // Session is genuinely gone (refresh failed/revoked) - get the
            // worker back to login instead of leaving them stranded on a
            // screen that will just keep silently 401'ing. Guarded so we
            // don't stack duplicate login screens if this fires from
            // several concurrent requests, or if they're already there.
            final currentRoute = CurrentRouteTracker.currentRouteName;
