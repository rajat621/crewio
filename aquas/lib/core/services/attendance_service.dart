...
class AttendanceService {
  ...
  Map<String, dynamic> _locationBody({
    double? lat,
    double? lng,
    double? accuracy,
    DateTime? timestamp,
    ...
    return {
      ...
      if (lat != null && lng != null && accuracy != null) 'accuracy': accuracy,
      // Always sent, with or without a GPS fix - this is what lets a
      // queued offline action (see AttendanceOfflineQueue) report the
      // moment it actually happened rather than the moment it finally
      // reached the server. The backend trusts this within a bounded
      // window (see resolveEventTimestamp in mobileLifecycle.controller.js).
  ...
  /// [startWork]).
  Future<LifecycleResult> checkIn({double? lat, double? lng, double? accuracy, DateTime? timestamp}) async {
    try {
      final response = await apiClient.post(
        '/api/mobile/attendance/check-in',
        data: _locationBody(lat: lat, lng: lng, accuracy: accuracy, timestamp: timestamp),
      );
  ...
  /// actually created.
  Future<LifecycleResult> startWork({double? lat, double? lng, double? accuracy, DateTime? timestamp}) async {
    try {
      final response = await apiClient.post(
  ...
  /// Worked" KPI.
  Future<LifecycleResult> stopWork({
    double? lat,
    double? lng,
    double? accuracy,
    double? hoursWorked,
    ...
    try {
      final body = _locationBody(lat: lat, lng: lng, accuracy: accuracy, timestamp: timestamp);
  ...
  /// WAITING_FOR_COMPANY.
  Future<LifecycleResult> siteFinished({double? lat, double? lng, double? accuracy, DateTime? timestamp}) async {
    try {
      final response = await apiClient.post(
