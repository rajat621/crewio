...
class _SalarySlipsPageState extends State<SalarySlipsPage> with ScreenshotProtection {
  ...
  Future<void> _openSlip(String slipId) async {
    try {
      ...
      if (!opened && mounted) {
        AppFeedbackDialog.showError(context, message: 'Could not open the salary slip.');
      }
    } catch (e) {
      if (!mounted) return;
      AppFeedbackDialog.showException(context, e, action: 'Could not open the salary slip');
    }
  }
...
Widget _buildYearChip({
  ...
  String _formatCurrency(double amount) => Formatters.formatAed(amount);
