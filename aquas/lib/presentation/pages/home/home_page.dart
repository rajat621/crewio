...
import '../../../core/notifications/notification_badge.dart';
import '../../../core/notifications/lifecycle_refresh_bus.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  final _attendanceOfflineQueue = ServiceLocator().attendanceOfflineQueue;
  final _salaryService = ServiceLocator().salaryService;
  final _secureStorage = const FlutterSecureStorage();

  bool _isWorking = false;
  bool _isOnLeave = false;
  bool _hasCheckedInValue = false;
  // Intercepting the existing field with a getter/setter means every
  // existing `_hasCheckedIn = true/false` call site elsewhere in this file
  // (checkIn, checkout/siteFinished, leave, lifecycle refresh) keeps
  // working unchanged, but now also drives BackgroundLocationService -
  // previously that service ran unconditionally for the lifetime of this
  // screen regardless of whether the worker was actually checked in,
  // which contradicts "stop location updates immediately after work ends"
  // (spec section 7). Now it only polls while genuinely checked in.
  bool get _hasCheckedIn => _hasCheckedInValue;
  set _hasCheckedIn(bool value) {
    _hasCheckedInValue = value;
    if (value) {
  ...
  void initState() {
    ...
    unawaited(ServiceLocator().notificationService.flushPendingActions());
    unawaited(ServiceLocator().notificationService.refreshUnreadCount());
    unawaited(_attendanceOfflineQueue.flush());
    LifecycleRefreshBus.tick.addListener(_onLifecycleRefreshPing);
    AttendanceSyncStatus.pendingCount.addListener(_onAttendanceSyncStatusChanged);
    AttendanceSyncStatus.lastSyncError.addListener(_onAttendanceSyncStatusChanged);

    unawaited(ServiceLocator().realtimeService.connect());
    // BackgroundLocationService is now started/stopped by the _hasCheckedIn
    // setter above, based on the worker's actual checked-in state (set by
    // _loadInitialState() below) - not unconditionally for the lifetime of
    // this screen.
  }
  ...
  Future<void> _loadInitialState() async {
    try {
      ...
      if (!mounted) return;
      AppFeedbackDialog.showException(context, e, action: 'Could not load your attendance');
    }
  }

  // Called when a site assignment/unassignment push arrives while this
  // screen is alive (see LifecycleRefreshBus / PushNotificationService).
  // Just re-runs the same fetch Home already does on open/pull-to-refresh
  // - lifecycleState from the backend is the single source of truth, so
  // there's nothing special to reconcile, just fetch again.
  void _onLifecycleRefreshPing() {
    if (!mounted) return;
    _loadInitialState();
  }

  void _onAttendanceSyncStatusChanged() {
    if (!mounted) return;
    setState(() {});
  }
  ...
  void dispose() {
    _timer?.cancel();
    BackgroundLocationService().stop();
    LifecycleRefreshBus.tick.removeListener(_onLifecycleRefreshPing);
    AttendanceSyncStatus.pendingCount.removeListener(_onAttendanceSyncStatusChanged);
    AttendanceSyncStatus.lastSyncError.removeListener(_onAttendanceSyncStatusChanged);
    super.dispose();
  }
  ...
  Future<void> _checkIn() async {
    ...
    try {
      ...
      if (AttendanceOfflineQueue.isConnectivityError(e)) {
        await _attendanceOfflineQueue.enqueueCheckIn();
        if (!mounted) return;
        setState(() {
          _hasCheckedIn = true;
          _isWorking = false;
        ...
        AppFeedbackDialog.showNotice(
          context,
          title: "You're offline",
          message: "Check-in will sync automatically once you're connected.",
        );
        return;
      }

      setState(() => _isSubmitting = false);
      AppFeedbackDialog.showException(context, e, action: 'Check-in failed');
    }
  }
  ...
  Future<void> _startWork() async {
    ...
    try {
      ...
      if (AttendanceOfflineQueue.isConnectivityError(e)) {
        await _attendanceOfflineQueue.enqueueStartWork();
        if (!mounted) return;
        setState(() {
          _isWorking = true;
          _isOnLeave = false;
        ...
        AppFeedbackDialog.showNotice(
          context,
          title: "You're offline",
          message: "This will sync automatically once you're connected.",
        );
        return;
      }

      setState(() => _isSubmitting = false);
      AppFeedbackDialog.showException(context, e, action: 'Could not start work');
    }
  }
  ...
  Future<void> _saveWorkSession(double pickedHours) async {
    try {
      ...
      if (AttendanceOfflineQueue.isConnectivityError(e)) {
        await _attendanceOfflineQueue.enqueueStopWork(hoursWorked: pickedHours);
        if (!mounted) return;
        setState(() {
          _isWorking = false;
          _hasCheckedIn = true;
        ...
        showDialog<void>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Text(
              "Recorded ${pickedHours.toStringAsFixed(1)} hours for today - this will sync "
              "automatically once you're connected, and the exact total will update then.",
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.of(context).pop(),
      ...
      _resumeWorkAfterDialogCancel();
      AppFeedbackDialog.showException(context, e, action: 'Could not save work session');
    }
  }
  ...
  Future<void> _siteFinished() async {
    ...
    try {
      ...
      if (AttendanceOfflineQueue.isConnectivityError(e)) {
        await _attendanceOfflineQueue.enqueueSiteFinished();
        if (!mounted) return;
        setState(() {
          _isSiteFinished = true;
          _isWorking = false;
      ...
      setState(() => _isSubmitting = false);
      AppFeedbackDialog.showException(context, e, action: 'Could not finish site');
    }
  }
  ...
  Future<void> _takeLeave() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _isSubmitting = false);
      AppFeedbackDialog.showException(context, e, action: 'Could not start leave');
    }
  }
  ...
  Future<void> _endLeave() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _isSubmitting = false);
      AppFeedbackDialog.showException(context, e, action: 'Could not end leave');
    }
  }

  // Shown whenever an attendance action is sitting in the offline queue
  // (see AttendanceOfflineQueue) - so it's visually obvious the on-screen
  // state hasn't been confirmed by the server yet, rather than looking
  // identical to a normal successful check-in/start/stop.
  Widget _buildAttendanceSyncBanner(double Function(double) w, double Function(double) h, double Function(double) f) {
    final pending = AttendanceSyncStatus.pendingCount.value;
    final error = AttendanceSyncStatus.lastSyncError.value;

    if (error != null) {
      return Container(
        width: double.infinity,
        color: AppColors.errorRed.withAlpha((0.12 * 255).round()),
        padding: EdgeInsets.symmetric(horizontal: w(16), vertical: h(8)),
        child: Row(
          children: [
            Icon(Icons.error_outline, size: f(16), color: AppColors.errorRed),
            SizedBox(width: w(8)),
            Expanded(
              child: Text(
                error,
                style: TextStyle(fontSize: f(11), color: AppColors.errorRed),
              ),
            ),
            TextButton(
              onPressed: () => AttendanceSyncStatus.lastSyncError.value = null,
              child: Text('Dismiss', style: TextStyle(fontSize: f(11))),
            ),
          ],
        ),
      );
    ...
    if (pending > 0) {
      return Container(
        width: double.infinity,
        color: AppColors.warningYellow.withAlpha((0.15 * 255).round()),
        padding: EdgeInsets.symmetric(horizontal: w(16), vertical: h(8)),
        child: Row(
          children: [
            SizedBox(
            ...
            Expanded(
              child: Text(
                pending == 1
                    ? '1 update will sync once you\'re connected'
                    : '$pending updates will sync once you\'re connected',
                style: TextStyle(fontSize: f(11), color: const Color(0xFF7A5B00)),
              ),
            ),
          ],
        ),
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          _buildAttendanceSyncBanner(w, h, f),
          Expanded(
            child: SafeArea(
              top: false,
  ...
  // Pull-to-refresh: re-fetches Home's lifecycle/attendance state from the
  // backend and flushes any offline-queued notification/attendance
  // actions, same as what happens automatically on reconnect - this gives
  // a manual, immediate way to "try now" instead of waiting.
  Future<void> _handlePullToRefresh() async {
    await Future.wait([
      _loadInitialState(),
      ServiceLocator().notificationService.flushPendingActions(),
      _attendanceOfflineQueue.flush(),
    ]);
    unawaited(ServiceLocator().notificationService.refreshUnreadCount());
  }
