...
import { Attendance, Company, Employee, User } from '../src/models/index.js';
...
const seedAttendance = async (employees, companies, owner) => {
...
const main = async () => {
  ...
  // ensure an owner user exists to attach companies/employees to
  let owner = await User.findOne({ email: 'owner@example.com' });
  if (!owner) {
    const password = 'Password123!';
    const hash = await bcrypt.hash(password, 10);
    owner = await User.create({ firstName: 'Owner', lastName: 'Dev', email: 'owner@example.com', passwordHash: hash, role: 'OWNER', isEmailVerified: true });
  ...
  const { employees, statusCounts: documentStatusCounts } = await seedEmployees(companies, owner);
  const attendanceStatusCounts = await seedAttendance(employees, companies, owner);

  // ensure owner's `company` points to a client company (not an owner/isOwner company)
  try {
    const clientCompany = companies.find((c) => c.companyRole === 'client');
    if (clientCompany) {
      owner.company = clientCompany._id;
      await owner.save();
      console.log('Owner user.company set to client company', clientCompany._id.toString());
    }
  } catch (err) {
    console.warn('Failed to set owner.company to client company', err.message);
  }
