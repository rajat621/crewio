...
export const reportLifecycleEvent = async ({
  ...
  pushTitle,
  pushBody,
  // The dashboard bell is scoped to check-ins only (see
  // notification.controller.js) - every other lifecycle event still fires
  // its real-time socket event, push, and audit log exactly as before,
  // it just no longer also persists a bell notification. Only the
  // check-in call site relies on the default (true).
  ...
  if (notifyDashboard && resolvedOwnerId && title) {
    try {
      await Notification.create({
        ...
        // Only check-in currently reaches this block (every other
        // lifecycle event now passes notifyDashboard: false) - if a
        // future event legitimately needs a dashboard bell entry too,
        // give it its own category value and update the dashboard query
        // in notification.controller.js to match.
        category: 'checkin',
      });
    } catch (err) {
