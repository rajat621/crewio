  ...
  try {
    ...
    await admin.messaging().send({
      ...
      android: {
        priority: 'high',
        notification: {
          // Must match the channel id created client-side in
          // PushNotificationService._initLocalNotifications() - otherwise
          // Android's automatic background/terminated display falls back
          // to a default/miscellaneous channel, which several OEMs
          // (OnePlus/Oppo ColorOS in particular) silently suppress or
          // downgrade unless the user has separately allowed it.
          channelId: 'crewcontrol_default',
          sound: 'default',
        },
      },
