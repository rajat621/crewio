...
/**
 * Phase 4.1 - universal human review gate.
 *
 * Whether review is required is now a SEPARATE question from extraction
 * quality. `validateExtractionQuality`'s approved/confident fields still
 * answer "how good does the system think this extraction is" - that's
 * unchanged, still useful, still shown to the reviewer as warning
 * severity. This function answers a different question: "does this
 * extraction need a human to look at it before it can become an
 * invoice." The answer is always yes, for any extraction that actually
 * produced rows to review - quality only ever changes what warnings
 * accompany that review, never whether it happens.
 *
 * The one legitimate exception is a genuinely non-reviewable technical
 * failure (nothing was extracted at all) - there's no data for a human
 * to review, so that case is handled separately as a hard failure by
 * the caller, not by this function.
 */
export const alwaysRequiresReview = (extractionData) => {
  const acceptedRows = extractionData?.accepted_rows || [];
