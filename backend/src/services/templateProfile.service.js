...
export const createTemplateProfile = async (companyId, data, userId) => {
  ...
  const profile = new TemplateProfile({
    ...
    ...profileData,
    // Must come AFTER the spread - see updateTemplateProfile's identical
    // fix for why: a client-supplied companyId/createdBy/lastModifiedBy/
    // version in profileData (req.body) would otherwise silently
    // override these, since later keys win in an object literal.
    // companyId specifically would let a client reassign this profile to
    // an arbitrary company, bypassing the controller's own req.params-
    // based validation entirely.
    companyId,
    templateName,
    version: nextVersion,
    createdBy: userId,
...
export const updateTemplateProfile = async (profileId, updates, userId) => {
  ...
  const newProfile = new TemplateProfile({
    ...
    ...updates,
    // These must come AFTER the spread - a client-supplied companyId,
    // createdBy, or lastModifiedBy in `updates` (req.body) would
    // otherwise silently override the intended value, since later keys
    // win in an object literal. companyId specifically would let a
    // client reassign this profile to a different tenant's company.
