...
import { objectExists, getObjectBuffer, driverFromStoredPath } from './storage.service.js';
...
export const tryResolveLogoBytes = async (company) => {
  ...
  // 2) logoFileId refers to a FileRecord-backed storage object (local or r2)
  //    D19.7 finding: this field has no current write path anywhere in the
  //    controllers (repository-wide search confirmed - only referenced in
  //    the Company/OwnerCompany schema definitions and this one read site),
  //    so it's not exploitable today. Scoped by ownerId anyway as
  //    defense-in-depth per the tenant-isolation standard in
  //    PROJECT_HANDOVER.md Section 4 ("prefer authorization at the
  //    database query/filter level"), so this stays safe automatically if
  //    a write path is ever added later without this file being revisited.
  if (company.logoFileId) {
    try {
      const fr = await FileRecord.findOne({ _id: company.logoFileId, ownerId: company.ownerId }).lean();
      if (fr && (fr.storageKey || fr.path)) {
        const driver = fr.storageDriver || driverFromStoredPath(fr.path);
        const key = fr.storageKey || fr.path;
        if (await objectExists({ key, driver })) {
          const bytes = await getObjectBuffer({ key, driver });
  ...
  // 3) company.logo might be a relative storage path / r2 key
  if (company.logo && typeof company.logo === 'string' && !company.logo.startsWith('http')) {
    try {
      const driver = driverFromStoredPath(company.logo);
      if (await objectExists({ key: company.logo, driver })) {
        const bytes = await getObjectBuffer({ key: company.logo, driver });
...
export const generateSalarySlipPdfBuffer = async (slipData, options = {}) => {
  ...
  if (logoAsset) {
    try {
      const optimized = await optimizeImageForPdf(logoAsset.bytes, logoAsset.mime);
      const isPng = optimized.mime && optimized.mime.includes('png');
      const img = isPng ? await pdfDoc.embedPng(optimized.bytes) : await pdfDoc.embedJpg(optimized.bytes);
  ...
  // Strip pdf-lib's default metadata (Producer/Creator/mod dates) - not
  // needed for a generated salary slip and it's pure dead weight.
  pdfDoc.setProducer('');
  pdfDoc.setCreator('');
  pdfDoc.setSubject('');
  pdfDoc.setKeywords([]);
