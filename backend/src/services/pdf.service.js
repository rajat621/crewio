...
import fs from 'fs';
import { reserveTempFilePath, saveLocalFile, cleanupTempFile } from './storage.service.js';

// NOTE: this module isn't currently wired up to any controller/route (the
// active salary-slip/invoice PDF flows go through Salaryslipjspdf.service.js
// and invoiceRenderer.service.js instead) - it's kept storage-abstraction
// clean here so re-enabling it later doesn't reintroduce a direct
// filesystem dependency.
export const generatePdf = async (data) => {
  const { clientDetails, invoiceDetails, items, vatRate, totalDeductions, companyId = null, ownerId = null } = data;
  ...
  const pdfPath = await reserveTempFilePath(`invoice_${invoiceDetails.invoiceNumber}.pdf`);
  ...
  return { pdfPath: saved.path, driver: saved.driver, key: saved.key };
};
