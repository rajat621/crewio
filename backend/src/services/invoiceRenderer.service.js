...
export const renderInvoicePdf = async ({
  ...
  const explicitVat = financials?.total_vat;
  const finalVat = Number(explicitVat != null && Number(explicitVat) > 0
    ? explicitVat
    : Number((finalSubtotal * Number(vatRate || 0)).toFixed(2)));
  ...
  const deductionBreakdown = financials?.deduction_breakdown && typeof financials.deduction_breakdown === 'object'
    ? financials.deduction_breakdown
    : {};
  const deductionBreakdownEntries = Object.entries(deductionBreakdown)
    .filter(([key, value]) => {
      const amount = Number(value || 0);
      return amount > 0 && !['total', 'subtotal', 'vat', 'net_payable', 'net_total'].includes(String(key).toLowerCase());
    })
    .map(([key, value]) => ({
      label: String(key)
        .replace(/_/g, ' ')
  ...
  if (tDataUri) {
    if (tDataUri.mime === 'application/pdf') {
      ...
      const optimizedT = await optimizeImageForPdf(tDataUri.bytes, tDataUri.mime, { maxDimension: 1600 });
      const tImg = optimizedT.mime.includes('png')
        ? await pdfDoc.embedPng(optimizedT.bytes)
        : await pdfDoc.embedJpg(optimizedT.bytes);
    ...
    if (tExt === '.pdf') {
      ...
      const optimizedT = await optimizeImageForPdf(imgBytes, tExt === '.png' ? 'image/png' : 'image/jpeg', { maxDimension: 1600 });
      const tImg = optimizedT.mime.includes('png')
        ? await pdfDoc.embedPng(optimizedT.bytes)
        : await pdfDoc.embedJpg(optimizedT.bytes);
      page.drawImage(tImg, { x: 0, y: 0, width: PAGE_W, height: PAGE_H });
  ...
  // Whitespace-only values (" ") are truthy in JS and would otherwise slip
  // past a plain `? :` check, printing a label with nothing after it.
  const hasText = (v) => typeof v === 'string' && v.trim() !== '';
  const clientLines = [
    hasText(client?.name)  ? `M/s. ${client.name.trim()}` : null,
    hasText(client?.poBox) ? `PO Box ${client.poBox.trim()}` : null,
    hasText(client?.phone) ? `Tel No ${client.phone.trim()}` : null,
    hasText(client?.fax)   ? `Fax no # ${client.fax.trim()}` : null,
    hasText(client?.address) ? client.address.trim() : null,
    hasText(client?.city)  ? client.city.trim() : null,
    hasText(client?.trn)   ? `TRN: ${client.trn.trim()}` : null,
  ].filter(Boolean);
  ...
  const DEDUCTION_DETAIL_ROW_H = 16;
  const TOTAL_ROW_H     = 18;
  const WORDS_ROW_H     = 24;
  const specialRowsH    = (deductionBreakdownEntries.length * DEDUCTION_DETAIL_ROW_H) + DEDUCTION_ROW_H + TOTAL_ROW_H + WORDS_ROW_H;
  ...
  const drawDeductionDetailRow = (label, amount, rowHeight) => {
    page.drawRectangle({
      x: TABLE_X, y: rowY - rowHeight,
      width: TOTAL_TABLE_W, height: rowHeight,
      borderColor: BORDER_C, borderWidth: BORDER_W,
    ...
    page.drawText(label, {
      x: TABLE_X + 4, y: labelTxtY,
      size: 8, font: fontRegular, color: rgb(0, 0, 0),
    });
    const amountWidth = cols.find((c) => c.key === 'amount')?.width || 55;
    const amountX = TABLE_X + labelSpanW;
    page.drawLine({
      start: { x: amountX, y: rowY },
      end:   { x: amountX, y: rowY - rowHeight },
      thickness: BORDER_W, color: BORDER_C,
    });
    const amountTxt = Number.isFinite(Number(amount)) ? Number(amount).toFixed(2) : '0.00';
    const amountTxtW = fontBold.widthOfTextAtSize(amountTxt, 8);
    page.drawText(amountTxt, {
      x: amountX + amountWidth - amountTxtW - 3,
      y: labelTxtY,
      size: 8,
      font: fontBold,
      color: rgb(0, 0, 0),
  ...
  for (const entry of deductionBreakdownEntries) {
    drawDeductionDetailRow(entry.label, entry.amount, DEDUCTION_DETAIL_ROW_H);
  }
  ...
  let regardsLineCount = 1; // name line already drawn
  if (company?.trn) {
    page.drawText(`TRN No. ${company.trn}`, {
      x: CL, y: thanksY - THANKS_LINE * (regardsLineCount + 1), size: THANKS_SIZE, font: fontBold, color: rgb(0, 0, 0),
    });
    regardsLineCount += 1;
  }
  // Full registered address, pulled from the owner's Company Profile page
  // (address + city + country), shown under the TRN line.
  const companyAddressLine = [company?.address, company?.city, company?.country]
    .map((part) => (part || '').trim())
    .filter(Boolean)
    .join(', ');
  if (companyAddressLine) {
    const ADDRESS_SIZE = 10;
    const wrappedAddress = wrapText(fontRegular, companyAddressLine, ADDRESS_SIZE, CR - CL);
    for (const addrLine of wrappedAddress.slice(0, 2)) {
      page.drawText(addrLine, {
        x: CL, y: thanksY - THANKS_LINE * (regardsLineCount + 1), size: ADDRESS_SIZE, font: fontRegular, color: rgb(0, 0, 0),
      });
      regardsLineCount += 1;
    }
  }
  ...
  const regardsLastLineY = thanksY - THANKS_LINE * regardsLineCount;
  const imageTopGap = 4;
  const signAsset = includeSignature ? imageBytesIfExists(signaturePath) : null;
  const stampAsset = includeStamp ? imageBytesIfExists(stampPath) : null;
  ...
  if (signAsset) {
    try {
      const optimizedSig = await optimizeImageForPdf(signAsset.bytes, signAsset.mime);
      sigEmbedded = optimizedSig.mime.includes('png')
        ? await pdfDoc.embedPng(optimizedSig.bytes)
        : await pdfDoc.embedJpg(optimizedSig.bytes);
  ...
  if (stampAsset) {
    try {
      const optimizedStamp = await optimizeImageForPdf(stampAsset.bytes, stampAsset.mime);
      stampEmbedded = optimizedStamp.mime.includes('png')
        ? await pdfDoc.embedPng(optimizedStamp.bytes)
        : await pdfDoc.embedJpg(optimizedStamp.bytes);
  ...
  // Strip pdf-lib's default metadata (Producer/Creator/mod dates) - pure
  // dead weight on a generated invoice.
  pdfDoc.setProducer('');
  pdfDoc.setCreator('');
  pdfDoc.setSubject('');
  pdfDoc.setKeywords([]);
