...
export const postEmployeeLocation = async (req, res) => {
  try {
    ...
    return serverError(res, 'Failed to record location');
  }
};
...
export const getEmployeeLocations = async (req, res) => {
  try {
    ...
    // EmployeeLocation has no virtuals/instance methods/middleware
    // (verified against models/EmployeeLocation.js).
    const items = await EmployeeLocation.find({ employee: employeeId, ownerId: user.ownerId }).sort({ createdAt: -1 }).limit(limit).lean();
...
export const getLatestEmployeeLocation = async (req, res) => {
  try {
    ...
    // .lean() - result goes straight to res.json(), never mutated.
    const latest = await EmployeeLocation.findOne({ employee: employeeId, ownerId: user.ownerId }).sort({ createdAt: -1 }).lean();
    return res.json({ message: 'Latest location retrieved', data: latest || null });
  } catch (error) {
    return serverError(res, 'Failed to fetch latest location');
  }
};
...
export const requestCurrentLocation = async (req, res) => {
  try {
    ...
    return serverError(res, 'Failed to request location');
  }
};
