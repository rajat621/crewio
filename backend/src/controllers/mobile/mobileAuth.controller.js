...
export const loginEmployee = async (req, res) => {
  try {
    ...
    // (e.g. after a legitimate phone upgrade). Checked only after password
    // verification succeeds (not before, as originally) so a wrong-password
    // guess against a guessed employeeId/appUserId can never reveal whether
    // that account exists via this check.
    ...
    return serverError(res, 'Employee login failed');
  }
};
...
export const refreshEmployeeToken = async (req, res) => {
  try {
    ...
    let decoded;
    try {
      decoded = jwt.verify(refreshToken, env.JWT_SECRET, { algorithms: ['HS256'] });
    } catch (_err) {
      return res.status(401).json({ message: 'Invalid or expired refresh token' });
    }
    ...
    return serverError(res, 'Failed to refresh token');
  }
};
...
export const logoutEmployee = async (req, res) => {
  try {
    if (req.employee?._id) {
      // Bump tokenVersion (invalidates every JWT issued before now) AND
      // clear the FCM token - otherwise this device keeps receiving this
      // employee's push notifications (salary slip, assignment, chat)
      // even after they've signed out of it, right up until someone else
      // logs in on it and overwrites the token.
...
export const registerPushToken = async (req, res) => {
  try {
    ...
    return serverError(res, 'Failed to register push token');
  }
};
