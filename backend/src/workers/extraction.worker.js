...
// // The job payload carries a storage reference (storageKey/storageDriver), not a
// // resolved local path - this worker may run in a separate process/container from
// // the API server that enqueued the job, so a temp file created there would not
// // exist here. Always resolve to a real local file right before the AI call, and
// // always clean it up afterwards. Falls back to treating payload.pdfPath itself
// // as the key (with driver inferred) for older jobs enqueued before this field
// // existed, so in-flight/queued jobs from before a deploy don't just start failing.
// const resolvePdfTempFile = async (payload) => {
//   const key = payload?.storageKey || payload?.pdfPath;
//   const driver = payload?.storageDriver || driverFromStoredPath(payload?.pdfPath);
//   return downloadToTempFile({ key, driver });
// };
...
//   // Resolve the source PDF to a real local filesystem path exactly once for this
//   // job (never pass an R2 key/object key as pdf_path to the AI service). On the
//   // r2 driver this downloads into a disposable OS temp file; on local it's a
//   // zero-copy no-op. The same local copy is reused across retries below, and is
//   // always deleted afterwards - success, failure, or dead-letter - via `finally`.
//   const pdfTempFile = await resolvePdfTempFile(payload);
...
const processAiJobInner = async (job) => {
  const { jobId, jobType, payload, traceContext, tenantId } = job.data || {};
  const requestId = traceContext?.requestId || null;
  const traceId = traceContext?.traceId || null;
...
// Thin wrapper so the tenant pending-count decrement always runs via
// finally, regardless of whether processAiJobInner succeeds or throws
// (AI-service timeout, provider failure, etc. are all expected, normal
// failure modes here). Without this, tenantJobCounts - an in-memory
// counter that only resets on a full worker restart - permanently
// leaked +1 per failed job, eventually causing every tenant who'd ever
// had a failure to be rejected by the TENANT_MAX_PENDING check.
const processAiJob = async (job) => {
  const tenantId = job.data?.tenantId;
  try {
    return await processAiJobInner(job);
  } finally {
    if (tenantId) {
      tenantJobCounts[tenantId] = Math.max(0, (tenantJobCounts[tenantId] || 1) - 1);
    }
...
function startJanitor() {
  ...
  janitorInterval = setInterval(() => {
    const proc = spawn('python', ['ai-service/scripts/janitor_temp_cleanup.py']);
    // Without this, a missing/misconfigured 'python' binary would crash
    // the entire worker process on spawn failure (ChildProcess is an
    // EventEmitter; an unhandled 'error' event throws) - every hour,
    // repeatedly, rather than just failing that one cleanup run.
    proc.on('error', (err) => {
      logEvent('temp_cleanup_spawn_error', { error: err.message });
    });
    proc.stdout.on('data', (data) => {
      try {
        const evt = JSON.parse(data.toString());
...
const bootstrap = async () => {
  ...
  // Without this, SIGTERM (sent by Railway/any platform on deploy or
  // restart) would kill this process abruptly mid-job rather than
  // letting BullMQ's own graceful close() wait for in-progress jobs to
  // finish - a job could otherwise be left stuck in 'active' status
  // forever, since nothing would ever mark it failed/retriable.
  const shutdown = async (signal) => {
    ...
    try {
      ...
      logEvent('worker_shutdown_error', { signal, error: err?.message || String(err) });
    } finally {
      process.exit(0);
    }
  };
