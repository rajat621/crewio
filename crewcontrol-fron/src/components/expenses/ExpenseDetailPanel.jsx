...
function MoneyFlowIcon({ direction }) {
  ...
  return (
    <div style={{
      width: 28, height: 28, borderRadius: "50%",
      border: `1px solid ${flowColor}`, background: "var(--bg-surface)",
      display: "flex", alignItems: "center", justifyContent: "center",
      position: "relative", flexShrink: 0,
    }}>
      <span style={{ fontSize: 11, fontWeight: 700, color: flowColor }}>$</span>
      <Arrow sx={{
        fontSize: 11, position: "absolute",
        top:    direction === "out" ? -4 : "auto",
        bottom: direction === "in"  ? -4 : "auto",
        right:  direction === "out" ? -4 : "auto",
        left:   direction === "in"  ? -4 : "auto",
        color: flowColor, background: "var(--bg-surface)", borderRadius: "50%",
      }} />
    </div>
  );
}
    ...
    <div style={{
      ...
      <div style={{ borderTop: `1px solid ${BORDER}`, flex: 1, overflow: "auto" }}>
        ...
        <div style={{ padding: "0 20px 20px", display: "flex", flexDirection: "column", gap: 10}}>
          ...
          {history.map((item, idx) => {
            const isDeductionEntry = isDeductionType(item.type);
            ...
            >
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                ...
                <div>
                  <p style={{ fontSize: 12, fontWeight: 400, color: flowColor, margin: 0 }}>
                    {item.label}
                  </p>
                  <p style={{ fontSize: 12, fontWeight: 400, color: flowColor, margin: 0 }}>
                    AED <strong>{Number(item.amount).toFixed(2)}</strong>
