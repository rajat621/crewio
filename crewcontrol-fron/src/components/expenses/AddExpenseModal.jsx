...
// Expense tab only ever records advances given to an employee now - the
// Fine/Penalty/Other-style deductions are entered from the Salary Slip's
// own Deduction section instead, which is where they're actually reconciled
// against a slip. Kept as a single-item list (rather than hardcoding the
// string in the JSX below) so anything reading EXPENSE_TYPES elsewhere
// doesn't need a separate change.
const EXPENSE_TYPES = ["Advance"];
...
export default function AddExpenseModal({ open, onClose, onSubmit, prefillEmployee }) {
  ...
  // Label shown on the submit button and sent to the API - always "Advance"
  // now that this modal only ever records advances.
  const expenseLabel = form.expenseType;
...
setForm((p) => ({
  ...
  rateHr:
    ...
    0,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <Field label="Expense Type">
              <div style={{ ...baseInput, display: "flex", alignItems: "center", color: DARK }}>
                Advance
              </div>
            </Field>
            <Field label="Amount">
