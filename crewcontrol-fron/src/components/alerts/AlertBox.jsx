...
import { useNavigate } from "react-router-dom";
import AlertAccordion from "./AlertAccordion";
import VatTaxAlert from "./VatTaxAlert";
...
function splitDocumentAlerts(items, defaultMeta = "Expiring soon") {
  const passport = [];
  const emirates = [];
  ...
  items.forEach((item) => {
    ...
    if (normalized.includes("passport")) {
      passport.push({
        ...item,
        name: name.replace(/'?s?\s*passport\s*(is\s*expiring\s*soon|has\s*expired)/i, "").trim() || name,
        meta: item?.meta || defaultMeta,
      });
    } else if (normalized.includes("emirates") || normalized.includes("emirate")) {
      emirates.push({
        ...item,
        name: name.replace(/'?s?\s*emirates?\s*id\s*(is\s*expiring\s*soon|has\s*expired)/i, "").trim() || name,
        meta: item?.meta || defaultMeta,
      });
    } else {
      passport.push(item);
...
function AlertBox({ alerts, onRefresh, isLoading }) {
  const navigate = useNavigate();
  const vatSummary = alerts?.vatSummary;
  const [openKey, setOpenKey] = useState(null);
  ...
  const handleMarkVatPaid = useCallback(async () => {
    setMarkPaidError("");
    setIsMarkingPaid(true);
    try {
      await companiesApi.markVatPaid();
      onRefresh?.();
    } catch (err) {
      setMarkPaidError(
        err?.response?.data?.message || "Failed to mark as paid. Please try again."
      );
    } finally {
  ...
  const groupedAlerts = useMemo(() => {
    ...
    const taxPayments = asList(alerts?.taxPayments);
    const documentExpiring = asList(alerts?.documentExpiring);
    const documentExpired = asList(alerts?.documentExpired);
    const siteFinished = asList(alerts?.siteFinished);
    const availableWorkers = asList(alerts?.availableWorkers);
    const salarySlips = asList(alerts?.salarySlips);
    const documents = splitDocumentAlerts(documentExpiring, "Expiring soon");
    const expiredDocuments = splitDocumentAlerts(documentExpired, "Expired");
    ...
    const documentSections = [
      ...
      },
      {
        ...
        actionLabel: "Update",
        items: expiredDocuments.passport,
        onAction: openEmployeeProfile,
      },
      {
        title: "Emirates ID Expired",
        actionLabel: "Update",
        items: expiredDocuments.emirates,
        onAction: openEmployeeProfile,
      },
    ];
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        // The KPI row and attendance chart render as soon as their own
        // (fast, pre-aggregated) queries resolve - Smart Alerts pulls from
        // several slower, secondary queries (full employee/company/
        // invoice/salary-slip lists) that are intentionally not blocking
        // that critical path (see useDashboardData.js). This is what fills
        // the panel's space while those finish, instead of a misleading
        // "nothing to report" empty state.
      ...
      >
        {groupedAlerts.map((group) => (
          <Box key={group.key} sx={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            <AlertAccordion
              title={group.title}
              count={group.count}
              isOpen={openKey === group.key}
              ...
              sections={group.sections}
            />
            {group.key === "daily-workforce-status" ? (
              <VatTaxAlert
                vatSummary={vatSummary}
                isOpen={isVatOpen}
                onToggle={() => setIsVatOpen((prev) => !prev)}
                onMarkPaid={handleMarkVatPaid}
