...
const S = {
  ...
  /* ── Table ───────────────────────────────────────────────── */
  // Total: 580px = 400 (expense type) + 180 (amount) — Receipt column removed
  table: {
    width: '100%',            // stretches to 580px (left column width)
    borderCollapse: 'separate',
      borderSpacing: '0 8px', 
  ...
  // Column widths sum to 580px
  thExpenseType: { width: '400px' },
  thAmount:      { width: '180px' },
...
const EXPENSE_GROUPS = [
  {
    ...
    items: [
      ...
      { label: 'ILOE',                          key: 'icn' },
      { label: 'Emigration Card Cancellation',  key: 'emigrationCancellation' },
      { label: 'Police Clearance Certificate',  key: 'policeClearanceCertificate' },
...
export const EmployeeExpensesTab = ({ employee = {}, expenses = {}, onUpdate = () => {} }) => {
  ...
  // "Total Investment" - live sum of this tab's own form fields while
  // editing (matches what's about to be saved as Employee.
  // totalInvestmentAmount server-side - see employee.controller.js's
  // updateEmployee).
  const totalInvestment = Object.values(formData).reduce(
    (sum, val) => sum + (parseFloat(val) || 0),
    0
  );
  // "Total Earned" - the running lifetime SalarySlip.netSalary total
  // (Employee.totalEarnedAmount), NOT derived from this tab's own fields.
  // Incremented server-side at salary-slip create/update/add-deduction
  // time (see salarySlip.controller.js), so it survives even if old
  // slips are ever purged for storage later.
  const totalEarned = Number(employee?.totalEarnedAmount) || 0;
    ...
    <div>
      ...
      >
        <div style={S.container}>
          ...
          <div style={S.rightColumn}>
            <div style={S.summaryCard}>
              <div style={S.summaryLabel}>Total Investment (AED)</div>
              <div style={S.summaryValueRow}>
                <div style={S.summaryValue}>{totalInvestment.toFixed(2)}</div>
              </div>
            </div>

            <div style={S.summaryCard}>
              <div style={S.summaryLabel}>Total Earned</div>
              <div style={S.summaryValueRow}>
                <div style={S.summaryValue}>{totalEarned.toFixed(2)}</div>
              </div>
            </div>
          </div>
        </div>
