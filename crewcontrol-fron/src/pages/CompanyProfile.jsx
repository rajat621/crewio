...
import { useOwnerCompany, useUpdateOwnerCompanyMutation } from "../hooks/useOwnerCompany";
...
const formatRegistrationMonth = (value) => {
  ...
  const label = new Date(Number(year), monthIndex, 1).toLocaleDateString("en-US", { month: "long" });
  return `${label} ${year}`;
};
...
function CompanyProfile() {
  ...
  // Shared with OnboardingCompanyProfile.jsx - same companiesApi.getOwnerCompany()
  // call, same queryKeys.companies.detail('owner') key, so the two pages
  // genuinely share this cache. The 404-is-valid distinction below reads
  // the query's own error object directly (the shared hook itself doesn't
  // branch on status code - OnboardingCompanyProfile.jsx never needed to
  // either), so this page's specific handling doesn't require changing
  // the shared hook.
  const { data: ownerCompany, error: ownerCompanyError, isLoading: isQueryLoading } = useOwnerCompany(true);
  const updateOwnerCompanyMutation = useUpdateOwnerCompanyMutation();
  ...
  useEffect(() => {
    ...
    if (ownerCompanyError) {
      if (ownerCompanyError.response?.status === 404) {
        // No owner company yet is a valid state. Keep the form editable and allow Save to create it.
        setError(null);
        setCompanyData((prev) => ({ ...prev, _id: "" }));
      } else {
        setError(ownerCompanyError.response?.data?.message || "Failed to load company data");
      }
      return;
    }
    ...
    if (ownerCompany) {
      ...
      setCompanyData({
        ...
        trn: company.trn || "",
        registrationMonth: company.registrationMonth || "",
  ...
  }, [ownerCompany, ownerCompanyError, isQueryLoading]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditingCompanyDetails) {
        ...
        if (companyData.registrationMonth !== originalData.registrationMonth) {
          updatePayload.registrationMonth = companyData.registrationMonth;
        }
        if (companyData.websiteLink !== originalData.websiteLink) {
          updatePayload.websiteLink = companyData.websiteLink.trim();
        }
      ...
      const ownerResp = await updateOwnerCompanyMutation.mutateAsync(updatePayload);
      ...
      // The block that used to follow here (a fallback to
      // companiesApi.updateCompany(companyData._id, ...) for "non-owner"
      // updates) was unreachable - the unconditional `return` above always
      // exits first, on every save, success or failure. Confirmed by
      // reading the code, not assumed: updatePayload.companyRole is
      // always forced to 'owner' a few lines up, and every save on this
      // page goes through updateOwnerCompany. Removed rather than kept as
      // dead code.
    } catch (err) {
                ...
                <Box>
                  ...
                  {isEditingCompanyDetails ? (
                    <TextField
                      fullWidth
                      type="month"
                      value={companyData.registrationMonth}
                    ...
                    <Typography sx={{ fontSize: 14, color: "var(--text-secondary)" }}>
                      {formatRegistrationMonth(companyData.registrationMonth)}
                    </Typography>
                  )}
                </Box>
