...
//                   onMouseLeave={(e) => (e.currentTarget.style.background = "#fff")}
//                 >
//                   <ArrowBackIosIcon sx={{ fontSize: 14, transform: "translateX(-1px)" }}  />
//                     Back
//                   </button>
//               )}
...
export default function AddCompany() {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      // Needed so "Assign employees" on the success screen can open the
      // same Unassigned Labor dialog used by the company list's 3-dot menu,
      // pre-scoped to the company that was just created.
      setCreatedCompanyId(response?.data?.data?._id || null);
      setIsSuccess(true);
    } catch (error) {
      setSaveError(error.response?.data?.message || "Failed to save company. Please try again.");
    } finally {
  ...
  const handleAssign = () => {
    // Same "Unassigned Labor" dialog the company list's 3-dot menu ->
    // "Assign Employee" opens, just pre-scoped to the company that was
    // just created here instead of an existing one.
  ...
  return (
    ...
    >
      ...
      <AssignEmployeeDialog
        ...
        onAssigned={handleEmployeesAssigned}
      />
    </>
  );
}