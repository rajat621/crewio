...
export default function OnboardingCompanyProfile() {
  ...
  const { data: ownerCompany } = useOwnerCompany(true);
  ...
  useEffect(() => {
    ...
    // Original only pre-filled once, on the initial successful fetch -
    // intentionally not re-running this sync every time `user` changes
    // (e.g. right after the updateUser call above), matching that.
  ...
  const handleNext = (e) => {
    ...
    setSaving(true);
    setSaveError("");
    updateOwnerCompanyMutation.mutateAsync(payload)
      .then((response) => {
