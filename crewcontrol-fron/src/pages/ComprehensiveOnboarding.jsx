...
import { LEGAL_DOCUMENT_LINKS } from "../api/legal";
...
export default function ComprehensiveOnboarding() {
  ...
  const { login, hasActiveAccess, isAuthenticated } = useAuth();
  const fileInputRef = useRef(null);

  // Email signup starts here unauthenticated (Account Details is step 1).
  // Google signup arrives here ALREADY authenticated - VerifyEmail.jsx
  // calls login(token, user) right after OTP verification succeeds, before
  // navigating to /signup - so this is what sends Google users straight to
  // the same Business Details step email users reach only after their own
  // Account Details + OTP steps, instead of duplicating this flow at a
  // second route. Only checked once, on mount - the email flow's own
  // internal step transitions (after ITS OTP step calls login()) are a
  // subsequent state update, not a remount, so they're unaffected.
  const [step, setStep] = useState(() => (isAuthenticated ? STEPS.BUSINESS : STEPS.ACCOUNT_DETAILS));
  ...
  // Synchronous guard against a rapid double-click saving twice - see
  // useSubmitGuard.js for why a ref beats a plain state flag here.
  const saveGuardRef = useRef(false);
  const [signupError, setSignupError] = useState("");
  const [otpError, setOtpError] = useState("");
  const [openLegalSlug, setOpenLegalSlug] = useState(null);
  ...
  const validateStep = useCallback(() => {
    ...
    if (step === STEPS.ACCOUNT_DETAILS) {
      ...
      if (!formData.legalAccepted) nextErrors.legalAccepted = "You must accept the legal agreements before creating your account.";
    }
  ...
  const handleLegalAcceptedChange = (e) => {
    const checked = e.target.checked;
    setFormData((prev) => ({ ...prev, legalAccepted: checked }));
    if (errors.legalAccepted) setErrors((prev) => ({ ...prev, legalAccepted: "" }));
    if (signupError) setSignupError("");
  };
  ...
  const handleFileChange = (file, fileType) => {
    ...
    if (fileType === "logo") {
      // Matches the original standalone Company Logo upload step exactly:
      // images only (no PDF), 2MB ceiling (stricter than stamp/signature/
      // template's 5MB image / 10MB PDF allowance).
      if (!isImage) {
      ...
      reader.onload = (e) => {
        ...
        setFormData((prev) => ({ ...prev, logo: dataUrl, logoPreview: dataUrl }));
      };
      reader.onerror = () => {
  ...
  const handleSave = async () => {
    ...
    // Save uploaded assets (logo/stamp/template/signature) to owner company record
    const updatePayload = {};
    ...
    if (formData.registrationMonth) updatePayload.registrationMonth = formData.registrationMonth;

    // These fields were previously collected in formData (Address/Contact
    // steps) but never copied into updatePayload, so the backend never saw
    // them - Company Profile then showed them empty until re-entered there
    // directly. Field names on the right match Company.js exactly (see
    // company.controller.js's updateOwnerCompany, which spreads any of
    // these straight onto the Company document).
    if (formData.website) updatePayload.websiteLink = formData.website.trim();
    if (formData.address) updatePayload.address = formData.address.trim();
    if (formData.city) updatePayload.city = formData.city.trim();
    if (formData.nationality) updatePayload.nationality = formData.nationality;
    if (formData.officialEmail) updatePayload.contactEmail = formData.officialEmail.trim();
    if (formData.countryIso) updatePayload.countryIso = formData.countryIso;
    if (formData.countryCode) updatePayload.countryCode = formData.countryCode;
    if (formData.mobile) updatePayload.mobileNumber = formData.mobile.trim();
    ...
    try {
      ...
      if (response.data?.data) {
        ...
        setFormData((prev) => ({ ...prev, ...{
          ...
          logoPreview: response.data.data.logo || prev.logoPreview,
  ...
  const handleContinue = () => {
    // New (non-lifetime) accounts must pass through the subscription page
    // before they ever reach Home; lifetime accounts skip it entirely.
    navigate(hasActiveAccess ? "/" : "/subscription");
  };
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      await authApi.signup({
        ...
        legalAccepted: formData.legalAccepted === true,
      });
      ...
      <div className="form-group">
        <label style={{ display: "flex", alignItems: "flex-start", gap: "8px", fontWeight: 400, cursor: "pointer" }}>
          <input
            ...
            onChange={handleLegalAcceptedChange}
            style={{ marginTop: "3px", width: "auto", flexShrink: 0 }}
          />
          <span style={{ fontSize: "13px", lineHeight: "19px", color: "#4B5563" }}>
            ...
            {LEGAL_DOCUMENT_LINKS.map((doc, idx) => (
              <span key={doc.slug}>
                <span
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    setOpenLegalSlug(doc.slug);
                  }}
                  style={{ color: "#2b4eff", cursor: "pointer", textDecoration: "underline" }}
                >
                  {doc.title}
                </span>
        ...
        {errors.legalAccepted && <p className="field-error">{errors.legalAccepted}</p>}
      </div>
      ...
      <div className="auth-card onboarding-card">
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={handleChange}
            className={errors.registrationMonth ? "onboarding-input-error" : ""}
          />
          {errors.registrationMonth && <p className="onboarding-field-error">{errors.registrationMonth}</p>}
        </div>
...
>
  ...
  const renderCompanyLogoUpload = () => (
    <div className="auth-wrapper onboarding-wrapper">
      ...
      <div className="auth-card onboarding-card">
        ...
        <div
          className={`onboarding-upload-area ${dragActive ? "drag-active" : ""} ${formData.logoPreview ? "has-preview" : ""}`}
        ...
        >
          {formData.logoPreview ? (
            <div className="logo-preview-container">
              <img src={formData.logoPreview} alt="Logo Preview" className="logo-preview-image" />
              <button
                type="button"
  ...
  const stepRenderers = {
    ...
    [STEPS.LOGO]: renderCompanyLogoUpload,
    [STEPS.STAMP]: renderStampUpload,
  ...
  return (
    <div className="auth-wrapper">
      ...
      <LegalDocumentModal
        slug={openLegalSlug}
        open={Boolean(openLegalSlug)}
        onClose={() => setOpenLegalSlug(null)}
      />
    </div>
  );
}
