...
const EXPENSE_KEY_MAP = {
  ...
  "Police Clearance Certificate": "policeClearanceCertificate",
...
const dropdownRef = useRef(null);
  ...
  const countries = COUNTRY_OPTIONS; // same value as getCountries() - already computed once at module load above, no need to recompute it
...
// getCities is async (lazy-loads the ~8MB world-city dataset on first use
// instead of it being in every page's bundle - see locationService.js) so
// this needs a loading state instead of a useMemo.
const [cityOptions, setCityOptions] = useState([]);
useEffect(() => {
  let active = true;
  getCities(data.nationality, data.state).then((cities) => {
    if (active) setCityOptions(cities);
  });
  return () => {
    active = false;
  };
}, [data.nationality, data.state]);
...
function Step2({ data, onChange, errors = {} }) {
  ...
  const workmenCompensationRef = useRef();
  const [dragging, setDragging] = useState(false);
  // useDocumentUpload's onSuccess callback is handed (fileUrl, fileName) -
  // only fileUrl was ever kept before (merged into `data` for the server),
  // discarding the name. Tracked here locally per field so the uploaded
  // state can show the actual filename instead of a generic label.
  ...
  const handleFile = (field, file) => {
    if (!file) return;
    // dataRef always holds the latest `data` prop even though this closure
    // is created once per render - onChange below must merge into whatever
    // is current when the upload resolves, not what was current when the
    // user picked the file (uploads can take a few seconds).
    upload(field, file, (fileUrl, name) => {
      setFileNames((prev) => ({ ...prev, [field]: name }));
      onChange({ ...dataRef.current, [field]: fileUrl });
    });
  };
  ...
  const handleRemove = (field) => {
    remove(field, () => {
      ...
      onChange({ ...dataRef.current, [field]: "" });
    });
  };

  const handleRetry = (field) => {
    retry(field, (fileUrl, name) => {
      setFileNames((prev) => ({ ...prev, [field]: name }));
      onChange({ ...dataRef.current, [field]: fileUrl });
    });
  };

  // Editing an existing employee: data[field] already holds a URL from a
  // previous session, but fileNames[field] is empty since nothing was
  // uploaded THIS session - fall back to the URL's own last path segment
  // rather than showing nothing.
  const displayFileName = (field) => {
    if (fileNames[field]) return fileNames[field];
    if (!data[field]) return "";
    try {
      const path = new URL(data[field], window.location.origin).pathname;
      return decodeURIComponent(path.split("/").pop() || "Document");
    } catch {
      return String(data[field]).split("/").pop() || "Document";
    }
  };

  const renderDropZone = (label, field, inputRef, placeholder) => {
    const fieldStatus = status[field] || (data[field] ? "success" : "idle");
    const isUploading = fieldStatus === "uploading";
    const hasError = fieldStatus === "error";
    const hasValue = Boolean(data[field]) && fieldStatus !== "error";

    return (
      <Field label={label}>
        {hasValue ? (
          // Uploaded state: same dashed-border box as the empty state, with
          // a file icon + the real filename in place of the drag prompt,
          // and Preview/Browse/Remove below it.
          <div
            style={{
          ...
          >
            ...
            <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
              <button
                type="button"
                onClick={() => employeesApi.previewDocument(data[field])}
                style={{
                  height: "34px",
                  padding: "0 20px",
                  background: "#fff",
          ...
          <div
            ...
            onDrop={(e) => {
              e.preventDefault();
              setDragging(false);
              if (isUploading) return; // prevent duplicate upload while one is in flight
              handleFile(field, e.dataTransfer.files[0]);
            }}
            style={{
              border: `1.5px dashed ${hasError ? "#DC2626" : dragging ? BLUE : "var(--border-input-hover)"}`,
              borderRadius: "10px",
              padding: "40px 24px",
              display: "flex",
              flexDirection: "column",
            ...
            {isUploading ? (
              <>
                <p style={{ fontSize: "14px", color: DARK, margin: 0 }}>
                  Uploading... {progress[field] || 0}%
                </p>
                <div style={{ width: "100%", maxWidth: "220px", height: "4px", background: "var(--border-card)", borderRadius: "999px", overflow: "hidden" }}>
                  <div style={{ width: `${progress[field] || 0}%`, height: "100%", background: BLUE, transition: "width 0.2s ease" }} />
                </div>
              </>
            ) : (
              <>
                <p style={{ fontSize: "14px", color: hasError ? "#DC2626" : DARK, margin: 0 }}>
                  {hasError ? uploadErrors[field] : placeholder}
                </p>
        ...
        <input
          ...
          onChange={(e) => {
            ...
            // Allow re-selecting the exact same file again (e.g. Replace
            // then pick the identical file) - without this, onChange
            // wouldn't fire a second time for an unchanged file list.
            e.target.value = "";
          }}
        />
      </Field>
  ...
  return (
    <div style={{ maxWidth: "560px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
        ...
        {renderDropZone("Upload Workmen Compensation Copy", "workmenCompensationCopy", workmenCompensationRef, "Drag & drop workmen compensation copy here")}
      </div>
    </div>
  );
}
...
function AddEmployee() {
  ...
  // Belt-and-suspenders alongside isSubmitting above: a plain state flag
  // can still let a very fast double-click slip through, since React
  // batches the re-render that would actually disable the button. This
  // ref is checked synchronously, before anything else runs, so a second
  // click while a submission is already in flight is a guaranteed no-op.
  const submitGuardRef = useRef(false);
  const [assignModalOpen, setAssignModalOpen] = useState(false);
  // Reused from Employees.jsx - identical params/fallback chain, genuine
  // cache sharing. React Query fetches on mount rather than only when the
  // assign modal opens, so by the time the user reaches this step (after
  // filling out the rest of the wizard) the data is very likely already
  // warm - a disclosed, beneficial side effect of the switch to a
  // declarative query, not something the original did.
  const { data: companyOptions = [], isLoading: companyLoading } = useActiveClientCompanies();
  ...
  // Defaults the selection to the first available company once the
  // (already query-cached, likely prefetched from mount) list resolves -
  // only when nothing is selected yet, so this doesn't reset a choice the
  // user already made. Matches the original's "default to first option"
  // intent without needing to force-reset on every modal open, which
  // wasn't the original's real intent - just an artifact of it
  // re-fetching (and therefore re-defaulting) every time the modal opened.
  useEffect(() => {
    if (!selectedCompanyId && companyOptions.length) {
  ...
  const handleAssignCompany = async () => {
    ...
    try {
      ...
      setAssignError("");
      await assignEmployeeMutation.mutateAsync({ employeeId: createdEmployeeId, companyId: selectedCompanyId });
    ...
    if (currentStep === 4) {
      ...
      try {
        ...
        const payload = {
          ...
          workmenCompensationCopy: step2Data.workmenCompensationCopy || null,
