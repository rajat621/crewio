  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "32px 32px 24px", display: "flex", gap: "20px", alignItems: "flex-start" }}>
        ...
        >
          <PersonIcon sx={{ fontSize: 38, color: SLIP_AVATAR_FG }} />
        </div>
        <div>
...
const nextNumber =
  ...
  const buildSlipData = () => {
    ...
    return {
      // TODO: replace with the logged-in company's actual profile data.
...
const slipPayload = {
  ...
  const handleDownloadPDF = () => {
    ...
    const pdf = generateSalarySlipPdf(dataToDownload);
    const safeName = (form.employeeName || "salary-slip").trim().replace(/\s+/g, "_");
    pdf.save(`${safeName}_Salary_Slip_${form.payMonth}.pdf`);
