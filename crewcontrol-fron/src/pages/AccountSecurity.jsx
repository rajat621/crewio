...
} from "../hooks/mutations/useAccountSecurityMutations";

function AccountSecurity() {
  const navigate = useNavigate();
  const { logout } = useAuth();

  const { data: meData, isLoading, error: meQueryError } = useMe();
  ...
  const { data: billingStatus = null, isLoading: isBillingLoading } = useBillingStatus();
  ...
  const manageBillingMutation = useManageBillingMutation();
  ...
  // Syncs local twoFactorEnabled from the fetched user, and surfaces a
  // load failure into the same shared `error` banner the mutation
  // handlers below also write to - matches the original's
  // loadSecurityState effect exactly (set once when the fetch resolves;
  // any later setError call from a user action, e.g. a failed password
  // change, simply overwrites this, same as before).
  useEffect(() => {
    if (meData) setTwoFactorEnabled(Boolean(meData?.twoFactorEnabled));
  }, [meData]);

  useEffect(() => {
    if (meQueryError) setError(meQueryError.response?.data?.message || "Failed to load security settings");
  }, [meQueryError]);
  ...
  const handleManageBilling = async () => {
    ...
    try {
      const { data } = await manageBillingMutation.mutateAsync();
      if (data?.url) {
        window.location.href = data.url;
      }
    } catch (err) {
      setBillingError(err.response?.data?.message || "Failed to open the billing portal");
      setIsOpeningPortal(false);
    }
  };

  const PLAN_LABELS = { plus: "Crewio Plus", pro: "Crewio Pro", ultra: "Crewio Ultra" };
  ...
  const handleVerifyTwoFactor = async () => {
    ...
    try {
      setIs2faBusy(true);
      setError(null);
      await verifyTwoFactorMutation.mutateAsync(twoFactorCode.trim());
      setTwoFactorEnabled(true);
      setShowTwoFactorSetup(false);
      setTwoFactorCode("");
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {isBillingLoading ? (
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <Box>
                ...
                <Typography sx={{ fontSize: "14px", color: "var(--text-secondary)", mt: "6px" }}>
                  This account has permanent premium access and doesn't use billing.
                </Typography>
              </Box>
            </Box>
          ) : (
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <Box>
                <Typography sx={{ fontSize: "14px", color: "var(--text-primary)", fontWeight: 500 }}>
                  {billingStatus?.subscriptionPlan
                    ? `${PLAN_LABELS[billingStatus.subscriptionPlan] || billingStatus.subscriptionPlan} · ${
                        billingStatus.subscriptionStatus === "active" ? "Active" : billingStatus.subscriptionStatus
                      }`
                    : "No active subscription"}
                </Typography>
                <Typography sx={{ fontSize: "14px", color: "var(--text-secondary)", mt: "6px" }}>
                  {billingStatus?.hasBillingHistory
                    ? "Manage your payment method, download invoices, or cancel your subscription."
                    : "Subscribe to a plan to manage billing here."}
                </Typography>
              </Box>
              ...
              {billingStatus?.hasBillingHistory ? (
                ...
                >
                  {isOpeningPortal ? <CircularProgress size={18} color="inherit" /> : "Manage Billing"}
                </Button>
              ) : (
                <Button
        ...
        >
          ...
          {LEGAL_DOCUMENT_LINKS.map((doc) => (
            <Box
              key={doc.slug}
              onClick={() => setOpenLegalSlug(doc.slug)}
              sx={{
                display: "flex",
                justifyContent: "space-between",
                alignItems: "center",
            ...
            >
              ...
              <Typography sx={{ fontSize: "13px", color: "var(--color-primary)" }}>View</Typography>
            </Box>
          ))}
        </Box>
                <Box
        ...
        >
          ...
          >
            <Box>
              ...
              <Typography sx={{ fontSize: "14px", color: "var(--text-secondary)", mt: "6px" }}>
                Your account will delete in 2-3 wording days
              </Typography>
            </Box>
      ...
      <Dialog open={isDeleteDialogOpen} onClose={handleCloseDeleteDialog} maxWidth="xs" fullWidth>
        <DialogTitle sx={{ fontSize: "16px", fontWeight: 600 }}>Delete your account</DialogTitle>
        <DialogContent>
          <Typography sx={{ fontSize: "14px", color: "var(--text-secondary)", mb: "16px" }}>
            This permanently deletes your account and all data associated with it, including
            companies and employees. This action cannot be undone.
          </Typography>
          ...
          <Typography sx={{ fontSize: "14px", color: "var(--text-primary)", mb: "8px", fontWeight: 500 }}>
        ...
        <DialogActions sx={{ px: "24px", pb: "20px" }}>
          ...
          >
            {isDeletingAccount ? <CircularProgress size={18} color="inherit" /> : "Delete Account"}
          </Button>
        </DialogActions>
      </Dialog>
