...
const Employees = () => {
  ...
  useEffect(() => {
    ...
    const refetchAttendance = () => loadAttendanceRecords(selectedAttendanceMonth);
    const refetchEmployees = () => loadEmployees();
    ...
    const employeeEvents = ["employee:assigned", "employee:unassigned", "employee:site_finished"];

    attendanceEvents.forEach((event) => socket.on(event, refetchAttendance));
    employeeEvents.forEach((event) => socket.on(event, refetchEmployees));

    return () => {
      attendanceEvents.forEach((event) => socket.off(event, refetchAttendance));
      employeeEvents.forEach((event) => socket.off(event, refetchEmployees));
    };
  }, [socket, loadAttendanceRecords, loadEmployees, selectedAttendanceMonth]);
  ...
  const handleReactivateEmployee = useCallback(
    async (row) => {
      if (!row?.apiId) return;
      try {
        await employeesApi.reactivateEmployee(row.apiId);
      } catch (err) {
        console.error("Failed to reactivate employee:", err?.response?.data || err.message);
      }
      await loadEmployees();
    },
    [loadEmployees]
  ...
  return (
    ...
    >
      ...
      {activeTab === "track" && <TrackEmployee rows={attendanceRows} />}
